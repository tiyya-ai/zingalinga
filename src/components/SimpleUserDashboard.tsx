...
export default function SimpleUserDashboard({ 
  ...
  const handleVideoPlay = (moduleId: string) => {
    ...
    if (module) {
      // Check access before opening player
  ...
  console.log('Real modules:', realModules);
  console.log('Real purchases:', realPurchases);
  console.log('Purchased modules:', purchasedModules);
                    ...
                    <CardBody className={`p-4 ${viewMode === 'list' ? 'flex flex-row items-center gap-4' : ''}`}>
                      <div className={`bg-gray-300 rounded-lg relative overflow-hidden cursor-pointer ${viewMode === 'list' ? 'w-32 h-20' : 'aspect-video mb-3'}`}>
                        ...
                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                          <Button
                            isIconOnly
                            className={`w-12 h-12 rounded-full bg-gradient-to-r ${selectedTheme.primary} text-white hover:scale-110 transition-transform`}
                        ...
                        <div className="absolute top-2 left-2 flex flex-col gap-1">
                          {!accessResult.hasAccess && (
                            <Chip 
                              size="sm" 
                              className="bg-red-500/90 text-white border-red-400 backdrop-blur-sm"
                          ...
                          {module.price === 0 && (
                            <Chip 
                              size="sm" 
                              className="bg-blue-500/90 text-white border-blue-400 backdrop-blur-sm"
                            >
                              Free
                            </Chip>
                          )}
                        ...
                        {module.isPremium && (
                          <div className="absolute top-2 right-2">
                            <Chip 
                              size="sm" 
                              className="bg-yellow-500/90 text-black border-yellow-400 backdrop-blur-sm"
